-- Demo seed verilerini yalnızca boş veritabanında yükle.
-- Böylece mevcut ortamlardaki gerçek veriler korunur.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM users LIMIT 1) OR EXISTS (SELECT 1 FROM tickets LIMIT 1) THEN
        RAISE NOTICE '[seed] Existing data detected; demo seed migration skipped.';
        RETURN;
    END IF;
    ...
    PERFORM setval(pg_get_serial_sequence('tickets', 'id'), (SELECT COALESCE(MAX(id), 1) FROM tickets));
